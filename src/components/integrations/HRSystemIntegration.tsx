  ...
  const handleSave = async () => {
    if (!provider || !apiKey) {
      toast({
        ...
        description: "Selecione o sistema de RH e informe a API Key",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.from("api_integrations").insert({
        ...
        metadata: {
          system_category: "hr",
          sync_types: ["admissions", "vacations", "benefits"],
        },
      });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure a conexão com seu sistema de gestão de pessoas para sincronizar admissões, férias e benefícios
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="hr-provider">Sistema de RH</Label>
          <Select value={provider} onValueChange={setProvider}>
            <SelectTrigger id="hr-provider">
        ...
        <div className="space-y-2">
          <Label htmlFor="api-key">API Key / Token de Acesso</Label>
          <Input
            id="api-key"
            type="password"
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">
              Ativar importação automática de dados de RH
            </p>
          </div>
          <Switch
            id="sync-enabled"
